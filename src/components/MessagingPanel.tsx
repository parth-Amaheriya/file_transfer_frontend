...
const MessagingPanel = ({ messages, peers, onSendMessage, disabled = false, emojiEnabled = true, mentionsEnabled = true }: MessagingPanelProps) => {
  ...
  return (
    <div className="flex h-[calc(100dvh-15rem)] min-h-0 flex-col overflow-visible rounded-lg border border-border bg-background shadow-xs">
      ...
      <div 
        ref={scrollRef} 
        className="flex-1 min-h-0 overflow-y-auto px-8 py-8 space-y-8 scrollbar-thin"
      >
        {visibleMessages.length === 0 && (
      ...
      <div className="flex-shrink-0 border-t border-black/5 px-2 py-2">
        <div className="relative">
              ...
              <div className="max-h-56 overflow-y-auto">
                {mentionSuggestions.map((peer, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className="max-w-[200px] truncate font-medium text-foreground" title={label}>{label}</span>
                      {peer.label && peer.label !== peer.identifier && (
                      <span className="max-w-[150px] truncate text-xs text-muted-foreground" title={`@${peer.identifier}`}>@{peer.identifier}</span>
                      )}
                    </button>
                  );
                })}
          ...
          {allowEmoji && emojiPickerOpen && (
            <div
              ref={emojiPickerRef}
              className="emoji-picker-popover absolute bottom-full left-0 z-[100] mb-2 overflow-visible rounded-lg"
            >
              <Picker
                data={data}
                onEmojiSelect={(emoji: EmojiSelect) => insertAtCaret(emoji.native)}
