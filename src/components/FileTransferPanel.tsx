import { Upload, FileImage, FileVideo, FileArchive, File, Check, ArrowUp, ArrowDown, X, Search, CheckCircle2, Circle, Users, Loader2, XCircle } from "lucide-react";
...
export interface FileItem {
  ...
  size: string;
  progress: number;
  status: "uploading" | "sending" | "receiving" | "pending_approval" | "approved" | "transferring" | "rejected" | "completed" | "failed" | "cancelled";
...
const FileTransferPanel = ({ peers, selectedPeerIds, onSelectionChange, onFileUpload, onCancelTransfer, files, disabled = false, maxFileSizeBytes }: FileTransferPanelProps) => {
  ...
  const getPeerDisplayName = (peer: DeviceDescriptor) => peer.label?.trim() || peer.identifier;
  ...
  const handleFileSelect = (selectedFiles: FileList | null) => {
    ...
    if (selectedPeerIds.length === 0) {
      toast.error("Select at least one recipient before sharing files.");
      return;
    }

    // Limit concurrent uploads to avoid overwhelming the data channel
    const maxConcurrentUploads = 3;
    const files = Array.from(selectedFiles);
    if (files.length > maxConcurrentUploads) {
      toast.info(`Uploading first ${maxConcurrentUploads} files. Additional files will be queued.`);
    }
    ...
    <div className="space-y-4">
            ...
            {selectedPeers.map((peer) => {
              ...
              return (
                <button
                  ...
                  className="group inline-flex max-w-full min-w-0 items-center gap-1.5 overflow-hidden rounded-full border border-primary/15 bg-primary/5 px-2 py-1 text-left transition-colors hover:bg-primary/10"
             ...
             ring-1 ring-border">
                  ...
                  <span className="min-w-0 max-w-[90px] truncate text-[11px] font-medium text-foreground">
                    {displayName}
                  </span>
      ...
      <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
        {[...files].reverse().map((file) => {
          ...
          const isTransferring = file.status === 'sending' || file.status === 'uploading' || file.status === 'receiving' || file.status === 'pending_approval' || file.status === 'approved' || file.status === 'transferring';
          const statusBadge =
            ...
            null;
          return (
            <div key={file.id} className="surface grid w-full min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 rounded-lg p-4 animate-fade-in overflow-hidden">
